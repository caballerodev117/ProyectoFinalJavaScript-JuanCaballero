...
// manejo promesas (fetch) - carga desde json local
    ...
    botonesCategorias.forEach(boton => {
        boton.addEventListener("click", (e) => {
            ...
            if (e.currentTarget.id != "todos") {
                ...
                tituloPrincipal.textContent = "Todos";
                mostrarProductos(productos);
                tituloPrincipal.scrollIntoView();
            }
...
function eliminarDelCarrito(e) {
    ...
    Toastify({
        text: "Sacaste el producto del carrito",
        duration: 2600,
        newWindow: false,
        close: true,
        gravity: "top", // `top` or `bottom`
...
function vaciarCarrito() {
    ...
    if(cantidad.innerText != 0){
        swal.fire({
            ...
            text: `Tienes ${cantidad.innerText} ${cantidad.innerText > 1 ? "productos" : "producto"} en el carrito.`,
            icon: 'warning',
            ...
            if(result.isConfirmed){
                Swal.fire({
                    text: `Tu carrito está vacío, Se eliminaron ${cantidad.innerText} productos.`,
                    icon: 'success',
                    background: '#bf7a3b',
                    color: '#27292b',
                    iconColor: 'green',
...
function buscarJuego(evt) {
    ...
    .then( productos => {
        ...
        if (array.length === 0 || input === "" || input == " "){
            array.length = 0;
            tituloPrincipal.textContent = `No se encontró ningun resultado "${input}"`;
            tituloPrincipal.scrollIntoView();
